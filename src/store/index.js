...
export default new Vuex.Store({
  ...
  actions: {
    make(data) {
      return new Promise(() => {
        axios.post(`http:///192.168.35.75:4000/certification/make`,
          { ppt: data.state.ppt })
          .then(response => {
    ...
    save(data) {
      return new Promise(() => {
        axios.post(`http://192.168.35.75:4000/certification/save`,
          { ppt: data.state.ppt })
          .then(response => {
            console.log("성공", response.data.ppt)
            this.state.ppt = response.data.ppt
    ...
    check(data) {
      return new Promise((resolve, reject) => {
        axios.post(`http://192.168.35.75:4000/certification/check`,
